...
sfpd2003 <- bludf(mycon, 'select * from BLUADMIN."sfpd_incident_2003"')

sfpd2003 <- na.omit(sfpd2003)

ggplot(sfpd2003, aes(x = PdDistrict, fill = DayOfWeek)) + 
...
ggplot(sfpd2003_southern, aes(x = PdDistrict, fill = DayOfWeek)) + 
  geom_bar(position = "dodge") +
  xlab("PdDistrict") +
  ylab("No. of incidents") +
...
ggplot(sfpd2003_southern, aes(x = Month, fill = DayOfWeek)) + 
  geom_bar(position = "dodge") +
  xlab("Month") +
  ylab("No. of incidents") +
  ggtitle("San Francisco Police Department Incidents 2003 - Southern District Data") 
